...
/**
 * VendorDashboard - improved version with safe UI updates and Home refresh
 * after changes.
 */
public class VendorDashboard extends JPanel {
        ...
        new Thread(() -> {
            ...
            try {
                ...
                if (rc >= 200 && rc < 300) {
                    ...
                    SwingUtilities.invokeLater(
                            () -> JOptionPane.showMessageDialog(this, "Failed to load products: " + rc + "\n" + body));
                }
            } catch (Exception ex) {
                ex.printStackTrace();
    ...
    private JPanel makeProductCard(JSONObject p) {
        ...
        del.addActionListener(e -> {
            int conf = JOptionPane.showConfirmDialog(this, "Delete product \"" + name + "\"?", "Confirm delete",
    ...
    private void openAddProductDialog() {
        ...
        int ok = JOptionPane.showConfirmDialog(this, form, "Add Product", JOptionPane.OK_CANCEL_OPTION,
    ...
    private void openEditProductDialog(JSONObject product) {
        ...
        int ok = JOptionPane.showConfirmDialog(this, form, "Edit Product", JOptionPane.OK_CANCEL_OPTION,
                JOptionPane.PLAIN_MESSAGE);
        if (ok == JOptionPane.OK_OPTION) {
            String validationError = form.validateInputs();
            if (validationError != null) {
            ...
            JSONObject body = form.toJson();
            // Merge edited fields into the original product JSON so server validation
            // passes
            ...
            if (productId > 0)
                editProductAsync(productId, merged);
        }
    }
        ...
        new Thread(() -> {
            ...
            try {
                ...
                if (rc >= 200 && rc < 300) {
                    ...
                    SwingUtilities.invokeLater(
                            () -> JOptionPane.showMessageDialog(this, "Failed to add product: " + rc + "\n" + resp));
                }
            } catch (Exception ex) {
                ex.printStackTrace();
        ...
        new Thread(() -> {
            ...
            try {
                ...
                if (rc >= 200 && rc < 300) {
                    ...
                    SwingUtilities.invokeLater(
                            () -> JOptionPane.showMessageDialog(this, "Failed to update product: " + rc + "\n" + resp));
                }
            } catch (Exception ex) {
                ex.printStackTrace();
    ...
    private static class ProductFormPanel extends JPanel {
        ...
        private void populate(JSONObject p) {
            ...
            originalPriceField.setText(p.has("originalPrice")
                    ? String.valueOf(p.optDouble("originalPrice", p.optDouble("original_price", 0.0)))
                    : "");
        ...
        String validateInputs() {
            ...
            if (!rcText.isEmpty()) {
                try {
                    Integer.parseInt(rcText);
                } catch (NumberFormatException e) {
                    return "Review count must be integer.";
                }
            }
            return null;
        }
        ...
        JSONObject toJson() {
            ...
            try {
                o.put("price", Double.parseDouble(priceField.getText().trim()));
            } catch (Exception ex) {
            }
            try {
                String op = originalPriceField.getText().trim();
                if (!op.isEmpty())
                    o.put("originalPrice", Double.parseDouble(op));
            } catch (Exception ex) {
            }
            ...
            try {
                ...
                if (!rf.isEmpty())
                    o.put("rating", Double.parseDouble(rf));
            } catch (Exception ex) {
            }
            try {
                String rc = reviewCountField.getText().trim();
                if (!rc.isEmpty())
                    o.put("reviewCount", Integer.parseInt(rc));
            } catch (Exception ex) {
            }
