...
public class BillingPanel extends JPanel {
    ...
    private JPanel makeBillingRow(JSONObject item) {
        // compute a height proportional to screen width (adjust the multiplier to
        // taste)
        ...
        // Async fetch for product details (name & price) — unchanged logic but updates
        // compact UI
        new Thread(() -> {
            try {
    ...
    private void recalcTotalAsync() {
        new Thread(() -> {
            ...
            synchronized (cartModel) {
                snapshot = List.copyOf(cartModel.getItems());
            }
    ...
    private void submitOrderAsync() {
        ...
        new Thread(() -> {
            try {
                ...
                // Post the wrapped body { items: [...] } using HttpUtil which throws on non-2xx
                ...
                try {
                    String resp = HttpUtil.postString("http://localhost:8080/orders", body.toString(), headers);
                    ...
                    SwingUtilities.invokeLater(() -> {
                        ...
                        // show feedback dialog (mock - not saved)
                        FeedbackDialog fd = new FeedbackDialog(parent);
                        fd.setVisible(true);
                        // After dialog closes, navigate home
                        parent.showPage("HOME");
                    });
                } catch (Exception ex) {
                    SwingUtilities.invokeLater(() -> {
                        JOptionPane.showMessageDialog(BillingPanel.this, "Order failed: " + ex.getMessage());
                        paymentCompletedBtn.setEnabled(true);
