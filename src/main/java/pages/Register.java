...
public class Register extends javax.swing.JFrame {
        ...
        private boolean registerCust(String firstName, String lastName, String email, String password,
                ...
                System.out.println(
                                "First: " + firstName + ", Last: " + lastName + ", Email: " + email + ", Password: "
                                                + password);
                try {
                        String endpoint = isVendor ? "http://localhost:8080/auth/register/vendor"
                        ...
                        if (isVendor) {
                                // helper compatibility: set role/userType when registering vendor
                                json.put("role", "vendor");
                                json.put("userType", "vendor");
                        }
                        try {
                                String resp = HttpUtil.postString(endpoint, json.toString(),
                                                java.util.Map.of("Content-Type", "application/json", "Accept",
                                                                "application/json"));
                                System.out.println("Response: " + resp);
                                return true;
                        } catch (Exception ex) {
                                // Log the exception and show a friendly message extracted from the
                                // server's JSON body when available (HttpUtil throws IOException with
                                // the response body appended).
                                logger.log(java.util.logging.Level.WARNING, "Registration error", ex);
                                String msg = "Registration failed";
                                try {
                                        String em = ex.getMessage();
                                        if (em != null) {
                                                // HttpUtil formats errors as: "HTTP <code>: <body>"
                                                int colon = em.indexOf(':');
                                                String bodyPart = colon >= 0 && colon + 1 < em.length()
                                                                ? em.substring(colon + 1).trim()
                                                                : em;
                                                // Attempt JSON parse to extract a "message" field
                                                try {
                                                        org.json.JSONObject err = new org.json.JSONObject(bodyPart);
                                                        msg = err.optString("message", bodyPart);
                                                } catch (Exception je) {
                                                        // not JSON - use the raw body or exception message
                                                        msg = bodyPart;
                                                }
                                        }
                                } catch (Throwable t) {
                                ...
                                final String shown = msg == null || msg.isBlank() ? "Registration failed" : msg;
                                javax.swing.SwingUtilities.invokeLater(() -> javax.swing.JOptionPane
                                                .showMessageDialog(Register.this, "Registration failed: " + shown));
                                return false;
                        }
        ...
        private void RegisterActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_RegisterActionPerformed
                ...
                if (success) {
                        JOptionPane.showMessageDialog(this, "User registered successfully. Please sign in.");
                        this.dispose();
                        // after registration, go back to login rather than opening main app
